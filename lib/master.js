...
var fns = {
	...
	set(path,item,options,callback) {
		...
		var patch = [{op:"add",path:path,value:item,uid:Path.uid()}];

		// Apply patch and notify
		applyChanges(store,patch);
		patch.forEach(pushUID);	// Stores the patch uid in history
		notify("/",patch);
		callback(null,item);
	},
	...
	del(path,callback) {
		...
		var changes = [{op:"remove",path:path,uid:Path.uid()}];
	...
	diff(path,changes,callback) {
		...
		changes = changes.filter(notUID);
		applyChanges(store,changes,path);
		changes.forEach(pushUID);
		notify(path||"/",changes);
